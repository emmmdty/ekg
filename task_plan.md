...
Phase 21（causal 误报诊断与下一机制选型）已完成。误差主因锁定为跨句 FP；ATLoss smoke 因 causal/subevent 全零止损，AFL/ATGL/NCRL 与 FP 方向不匹配而排除。周四前不再运行第三个通用 loss；后续方法周期冻结为 pair-specific evidence/长文共同上下文重构。
...
- [x] 在 5090 完成 Torch/公式负控、显式 CUDA backward 与 P1 r14 validate-only。
- [x] seed13 2ep smoke 的 causal/subevent 均为 0，按预注册行为门止损；full/official/额外 seed 未启动。
- [x] 复核 AFL/ATGL/NCRL 后停止通用 loss 线；后续表示方法留待独立周期，不在周四前伪适配。
- **Status:** complete
