...
### 约束分级：一次真实的误判（2026-08-29）

`PHASE_A3` 契约里有一句「4090 上每次只运行一个实验任务」。我把它当成必须遵守的红线，
于是 `official_joint`（~100 epochs）与 `official_single`（50 epochs）被排成一条线，
**而四张卡里三张全空**。作者纠正：能否并行取决于 GPU 是否空闲，不取决于协议。

**为什么这是误判**：那句话保护的是「资源不打架」。两个 baseline 用不同的卡、不同的
run-dir、不同的进程、固定的 seed、只读的数据，彼此没有任何共享状态——**并行不改变
任何一个数字**。它是操作性约定，不是有效性约束。

**教训比这条具体的错更重要**：我把哈希集合解耦了（让工程修复不再作废信任根），
却没有解耦**自己的判断**——继续把写在文件里的每一句话当成同等强度的命令。
机制上的解耦 + 判断上的照章办事 = 没有解耦。

分级标准与三类清单见 `CLAUDE.md` / `AGENTS.md` 的「约束分级」一节。
判据只有一句：**不遵守它，是会让结论变错，还是只会让进度变慢？**
