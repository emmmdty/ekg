...
## ★★ D3.0 强 baseline 同协议闭合：**我们没有超过对手，只是与之不可区分**（2026-08-30）

论文原报的 45.4 / 47.1 / 47.6 在 **hidden test** 上；本文件前面的功效分析已经证明那三者
之间 2.2 点的差距小于单系统 CI 半宽，**跨 split 比较不可判别**。因此主表必须自跑对手。

**同协议**：P1 manifests（train 2,622 / internal-dev 291）｜seed 13｜12 epoch 等预算｜
lr 2e-5｜**同样的 α=0.5 逆频加权**｜同一内容寻址 roberta-base｜final-valid 未访问。
两条对手与我们的检测器架构不同族，其余由构造相同。

> ⚠️ 对手用与我们相同的 α 是**刻意**的：平凡 CE 会让它塌到 94.9% 的 CT+ 多数类（macro .1947），
> 拿那个当"对手"是给自己立稻草人（A 类红线「不拿更弱的对手充数」）。

### 主表（五类 macro-F1 @ internal-dev 291 篇）

| 系统 | 选中 epoch | **macro-F1** | CT+ | PS+ | CT− | **PS−** | Uu |
|---|---|---|---|---|---|---|---|
| **我们 `evidence`（耦合机制）** | 9 | **.5554** | .981 | .575 | .721 | **.500** | .000 |
| **RoBERTa+CLS**（同协议重跑） | 7 | **.5458** | .975 | .543 | .729 | .483 | .000 |
| **DMRoBERTa**（同协议重跑） | 7 | **.5423** | .978 | .588 | .669 | .476 | .000 |
| 我们 `uniform`（容量对照） | 4 | .5292 | .978 | .591 | .676 | .400 | .000 |
| 我们 `none`（复现底座） | 4 | .5283 | .979 | .599 | .653 | .350 | .061 |

### ★ 配对检验：三组全部平局

按预注册规则（配对 document-cluster bootstrap 2,000 次，CI 下界 > 0 才算胜出）：

| 对比 | Δ 五类 macro | 配对 95% CI | 判定 |
|---|---|---|---|
| 我们 `evidence` − RoBERTa+CLS | **+.0096** | [−.0398, +.0630] | ❌ 含 0 |
| 我们 `evidence` − DMRoBERTa | **+.0131** | [−.0250, +.0522] | ❌ 含 0 |

| 我们 `none`（复现底座） − RoBERTa+CLS | **−.0174** | [−.0727, +.0369] | ❌ 含 0 |

⇒ **不得写"超过 RoBERTa+CLS / DMRoBERTa"**，只能写"与之不可区分"。
反过来也成立：**连"我们的底座输给对手"都不可判别**——五个系统两两之间全是平局。
这与本文件前面算出的 internal-dev 配对 MDE（≈ +.051）完全一致：观测到的 +.01 量级效应
**在这个 split 上按构造就测不出来**。

### 三条必须一起说的事

1. **我们的复现底座（.5283）低于两条对手**（.5458 / .5423）。名义领先只发生在耦合机制臂上，
   而那点领先又不可判别。**Ch3 目前没有"超过多个公开方法"的结论。**
2. **对手不是稻草人**：RoBERTa+CLS 在最难的 PS− 上拿到 .483，与我们的 .500 几乎持平，
   远高于我们底座的 .350。它是一条真正的强线。
3. **五个系统全部在 Uu 上得零**（n=14）。该类在这个 split 上没有任何系统能做对，
   而它占五类 macro 的 1/5 —— 这是**指标本身的问题**，不是某个方法的问题。

### 由此得到的一个方法论结论（本章最硬的一块）

在 MAVEN-FACT 的 valid 规模上，**五个同协议系统（我们三臂 + 两条公开对手）的
macro-F1 全部落在 .528–.555 的区间内，而配对 MDE 是 ±.05** —— 也就是说
**这个基准在这个规模上无法区分任何两个方法**。论文原报 45.4/47.1/47.6 之间 2.2 点的
差距同样落在噪声内，与我们的实测互相印证。

这不是"我们没做出来"的托词，而是一个可以写进论文、并且**用自跑对手证实过**的观察：
该基准的主指标由 72 个稀有类实例决定，任何小于 ±.05 的 macro-F1 差异都不该被解读。
后续要么改判定轴（证据轴有 1,276 个 gold span，支持数是标签轴的 18 倍），
要么承认本章只能给出描述性对比。

### 产物

`runs/stages/D3/baselines/{cls,dynamic_multi}/seed-13/`：checkpoint、`baseline_config.json`、
...
本轮触发了 **Gate 1**（`EXPERIMENT_PLAN.md` §5，**在看到任何 D4 结果之前就已冻结**）。Gate 1 的
「不过」分支写得很死：**不启动第二个周期，直接进 Gate 2 讨论结构**。

所以本轮的裁决不是「再试一次」：**typed-cue 机制家族到此为止**，Ch3 的 D4 线以 `failed` 身份关闭，
下一步是把 A4（Ch4）与 C5（Ch5）跑出结果，在 Gate 2 上一并判还剩几个方法章。
`PHASE_D4` 的 stop conditions 允许「两个有效周期后封存」，但主表的 Gate 1 更具体且先于结果冻结，
**以主表为准**——这正是 Gate 存在的意义：结果难看时不临时给自己加一次机会。

**不做的事**：不调 threshold、不扫 epoch、不换 split、不加大 backbone、**也不开第 2 个周期**。
上面那个五维瓶颈假设**只作为记录**留给 Gate 2 之后的重规划，本阶段不据此改设计。
