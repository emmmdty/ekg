...
## ★★ C-11 H1 抑制上界：**方向可行，但相似度桶本身不是信号**（2026-09-18，本地纯 CPU，未训练、未占卡）

**主表 ID C-11**（`EXPERIMENT_PLAN.md` §4.1，2026-09-18 裁决 ① 批准 C5 第二设计周期后由 §8 候补区出列）。

### 开工自审

1. **科研价值**：对准第 5 章主表的 precision 缺口。G-5b 已证 `full` 与主锚的 1.084 **100% 在
   precision**（.756630 vs .788430，recall 已反超 .846422 vs .832461），并给出 H1：误合并集中在
   trigger 相似度 ≥0.8 的跨句桶。第二设计周期若要把角色相容性接到**抑制侧**，先得知道
   **抑制最多能买回多少**——买不回主锚就不值得训。证据在本页 G-5b 节；
2. **可行性**：数据（gold `internal-dev.jsonl` + 三臂 `predictions.jsonl`）、代码（官方 evaluator
   已钉在 `data/protocols/v6/tools/`）、算力（纯 CPU，两条命令各几十秒）全部在本地，无授权项。

### 怎么算的（两条命令，可重跑）

抑制只能**拆**已预测的簇，所以上界可以不训练就算出来。新脚本
`scripts/report_coref_suppression_bound.py` 对每个预测簇取其存活对的**连通分量**重新成簇，
再交官方 evaluator 打分；**no-op 行必须逐位复现该臂已登记的 MUC F1，否则中止**（两臂都过）。
...
uv run python scripts/report_coref_suppression_bound.py \
    --evaluator data/protocols/v6/tools/maven_ere_evaluate.py \
    --gold $R/preflight-r2/data/MAVEN_ERE/internal-dev.jsonl \
    --pred $R/pilot-r2/full/predictions.jsonl \
    --expect-muc-f1 79.9011532125206 --anchor-muc-f1 80.98472 \
    --output $R/suppression-bound-20260918/full.json          # remove_core 同样一条
```

两种抑制器：**oracle** 只否决桶里**确实错**的对（无人能做得更好，recall 代价为 0）；
**blind** 把桶里的对**不分对错全否**（同一条规则、没有先知，它的 recall 损失就是真实代价）。

### 主表：H1 桶（相似度 ≥0.8 · 跨句）的抑制上界

| 臂 | 抑制器 | 否决 错/对 | MUC P | MUC R | MUC F1 | vs 主锚 80.98472 |
|---|---|---:|---:|---:|---:|---:|
| `full` | 无（no-op 校验） | 0 / 0 | .756630 | .846422 | **79.90115** | −1.084 |
| `full` | **oracle** | 213 / 0 | .846422 | .846422 | **84.64223** | **+3.658** |
| `full` | blind | 213 / 699 | .787440 | .568935 | 66.05876 | −14.926 |
| `remove_core` | 无（no-op 校验） | 0 / 0 | .763371 | .821990 | 79.15966 | −1.825 |
| `remove_core` | **oracle** | 210 / 0 | .851718 | .821990 | **83.65897** | +2.674 |
| `remove_core` | blind | 210 / 675 | .795000 | .554974 | 65.36485 | −15.620 |

放宽到任意句对、任意阈值只会把 oracle 抬得更高（`full` 在 ≥0.2 / 任意句对处到 88.58447），
**上界不是瓶颈**。

### 决定第二周期值不值得训的那条曲线：误否决预算

oracle 的 213 个正确否决**固定不动**，再随机加入 N 个误否决（桶内共 699 个正确对），5 次抽样：

| 误否决 N | MUC F1 均值 | min | max | vs `full` 79.90115 | vs 主锚 80.98472 |
|---:|---:|---:|---:|---:|---:|
| 0 | 84.64223 | — | — | +4.741 | +3.658 |
| 100 | 83.68774 | 83.31862 | 83.93327 | +3.787 | +2.703 |
| 200 | 82.67559 | 82.17469 | 83.11229 | +2.774 | +1.691 |
| **300** | **80.98439** | 80.36199 | 81.75313 | +1.083 | **−0.000** |
| 400 | 78.64220 | 77.59259 | 80.03630 | −1.259 | −2.343 |
| 699（= blind） | 66.05876 | — | — | −13.842 | −14.926 |

### 三条结论

1. **H1 成立，且上界够得到主锚**——桶里装着 213 个误合并对，完美抑制它们把 `full` 从 79.90115
   抬到 **84.64223**（+4.741），比主锚高 3.658。**接到抑制侧这个方向不是死路**；
2. **但桶本身不是信号**。桶内 213 错 / 699 对，**基率只有 23.36% 是错的**；blind 规则把 MUC
   recall 从 .846422 打到 .568935，F1 掉到 66.06。**仅仅追平主锚**就要求在完美召回 213 个错对的
   前提下误否决 ≤300 个正确对 ⇒ 桶内精度 ≥ **213/513 = 41.5%**，约为基率的 **1.8 倍**；
   而 oracle 召回本身已是慷慨假设，真实要求只会更高。**⇒ 第二周期的增益必须真的来自角色相容性，
   不能来自相似度桶**；
3. **这个集中不是机制造成的**。`remove_core` 的桶内占比 238/398 = **59.8%**，与 `full` 的
   243/419 = 58.0% 基本相同，**低的是主锚**（203/394 = 51.5%）。即高相似度误合并是**我们这条
   coref head 的共同性质**，不是 `full` 新引入的。`full` 相对 `remove_core` 仍多约 1 点 oracle
   余量（84.642 vs 83.659），在同样 300 个误否决下也仍高约 1 点（80.984 vs 79.992）。

### 限制（三条，随结论一起报）

1. **上界不是增益**。oracle 假设「桶内错对全部召回、正确对一个不误伤」，任何真实抑制器都做不到；
   本节给的是**可行性的天花板与预算**，不是预期收益；
2. **误否决按随机抽样定价**（seed 13 起 5 次抽样，表里给了 min/max）。真实抑制器的误否决不是
   随机的，可能系统性地集中在某类正确对上，那时同样的 N 代价更大；
3. 仍是**单 seed 的两条 pilot 曲线**，本 split 的 MUC 噪声地板至今未量（D4 的 ±.01 是 macro-F1
   的地板，不能搬）。**主锚预测文件不在本地**（在 5090），故主锚只作为数值判据出现、未跑本节分析。

### 下一步

第二周期的设计现在有了**一条量化验收线**，而不只是方向：角色相容性作抑制信号，
**必须在 ≥0.8 跨句桶上做到精度 ≥41.5% 才仅仅追平主锚**（基率 23.36%）。
达不到就不该训——这条可以在训练前用现有 sidecar 特征离线估一遍。
⛔ 本节不触发训练、不申请 GPU、不动任何契约。

---

## ★★★ C-12 桶内判别力：**现有角色信号付不起 C-11 的价钱**（2026-09-18，本地纯 CPU，未训练、未占卡）

**主表 ID C-12**（`EXPERIMENT_PLAN.md` §4.1）。C-11 把第二周期变成一个数：桶内精度要 ≥41.5%。
这一节问的是**在上卡之前就能答的那半**——我们**已经有的**角色相容性信号，在那个桶上有没有这个精度。

### 开工自审

1. **科研价值**：直接决定 C5 第二设计周期**值不值得上卡**。验收线来自本页 C-11 节，
   信号来自 `src/ekg/nodes/role_uncertainty.py` 里**已实现且已在 `full` 臂里训过**的那套特征；
2. **可行性**：唯一缺的输入是论元预测（`qwen3-full-r3/merged/predictions.jsonl`），在 4090 上；
   双端 `sha256` 核过 `855906d39e71d5cef838c3a72515ef837803afd6a43c88e6879274bf72e7142a`
   后搬回本地 `upstream-arguments/`（24.7 MB，实测 38 秒）。其余全在本地，纯 CPU。

### 怎么算的（一条命令，可重跑）

`scripts/estimate_role_suppression_signal.py`：对 C-11 那 912 个桶内对（213 错 / 699 对），
用**不训练、不调阈值**的两种用法把角色信号当抑制器，再用 C-11 的 `score_veto` 重新成簇、
交官方 evaluator 打分——与上界表同轴。

- **rule**：否决 `incompatible_role_merges` 规则实际触发的那些对（某个角色两边都有填充物却无共享内容词）；
- **ranking**：按**角色一致度**（两边都填了的角色上的 token Jaccard 均值）从弱到强否决前 k 个；
  **没有可比角色的对排在最后**——抽取器不作答不是证据。

```bash
R=runs/stages/C5/c5-v61-argument-uncertainty-5090-r1
uv run python scripts/estimate_role_suppression_signal.py \
    --evaluator data/protocols/v6/tools/maven_ere_evaluate.py \
    --gold $R/preflight-r2/data/MAVEN_ERE/internal-dev.jsonl \
    --pred $R/pilot-r2/full/predictions.jsonl \
    --arguments $R/upstream-arguments/qwen3-full-r3.predictions.jsonl \
    --arm-muc-f1 79.9011532125206 --anchor-muc-f1 80.98472 \
    --output $R/suppression-bound-20260918/signal-full.json
```

### 主表：桶内 912 对，213 错，**基率 23.36%**

| 抑制器 | 否决 | 其中真错 | **桶内精度** | MUC F1 | vs `full` 79.90115 | vs 主锚 80.98472 |
|---|---:|---:|---:|---:|---:|---:|
| **rule · incompatible** | 618 | 144 | **.2330** | 74.40147 | −5.500 | −6.583 |
| ranking @50 | 50 | 12 | .2400 | **79.56811** | −0.333 | −1.417 |
| ranking @100 | 100 | 27 | **.2700** | 79.16318 | −0.738 | −1.822 |
| ranking @200 | 200 | 58 | .2900 | 78.18648 | −1.715 | −2.798 |
| ranking @300 | 300 | 75 | .2500 | 77.25694 | −2.644 | −3.728 |
| ranking @513（C-11 的预算点） | 513 | 124 | .2417 | 75.15808 | −4.743 | −5.827 |
| ranking @912（= 全桶 blind） | 912 | 213 | .2336 | 66.05876 | −13.842 | −14.926 |

### 三条结论

1. **现有角色信号在这个桶上几乎没有判别力**。所有操作点的桶内精度都在 **.2330–.2900**，
   基率是 **.2336**——最好的一档（@100 的 .2700）也只比乱选高 3.6 个百分点，
   离验收线 **41.5%** 差得很远。**没有任何一档跑赢 `full` 自己**，更不用说主锚；
2. **注册中介规则 `incompatible` 在这里是失效的**：它在 912 对里触发 **618 次（67.8%）**，
   精度 **.2330 低于基率**——即它标出的「角色不相容」与「这一对是不是误合并」**基本无关**。
   机制层面的原因可读：同一事件的两次 mention 常由抽取器给出**字面不同**的填充物
   （"the militia" / "rebel fighters"），规则把这判成不相容；
3. **⇒ 问题在信号源，不在接法**。H1 的补救假设是「把角色相容性接到抑制侧」，
   但 C-11 已证抑制侧**有 4.74 分的空间**，C-12 证**这套特征填不满它**。
   `full` 本身就是一个已经训过这些特征的模型（作正残差抬分），换个方向用同一批信息不会变出判别力。
   **第二周期若要开，必须换信号源**（不是换接法、不是调阈值、不是加 epoch）。

### 限制（三条，随结论一起报）

1. **这测的是未训练的用法**。一个把 13 维特征与上下文一起训的头**可能**比排序规则强；
   但本节量的是这批特征**在这个桶上的信息量**，而 `full` 已经是训过它们的模型，两头都指向同一结论。
   要推翻它，需要证明存在一种训练方式能把 .2336 的基率拉到 .415 以上；
2. **只在 `full` 臂、单 seed、internal-dev 291 篇上做**；本 split 的 MUC 噪声地板仍未量过；
3. ⛔ **不得外推成「角色相容性没有价值」**。本节的断言只覆盖
   **「≥0.8 相似度 · 跨句 · 已被 `full` 合并」**这个桶，以及 `qwen3-full-r3` 这一份论元产物。

### 下一步：交作者裁决

裁决 ① 批准的是 **H1 这一条接法**。C-11 + C-12 合起来把它**证伪**了：方向有空间，信号填不满。
按 `CLAUDE.md` 的止损纪律，**换信号源＝新机制家族，必须回 R1 证明文献差异、因果链、
≥80% 目标 power 与单变量消融**，不在本轮授权范围内。⇒ **停下交裁决**，两个选项：

- **(甲)** C5 第二周期就此收，Ch5 并入「负结果与归因」章——**这正是裁决 ① 写好的兜底形态**
  （`Ch6 承重 + 一章负结果归因`），且 C-11/C-12 本身就是有信息量的归因证据；
...
### 实测结果（2026-09-18 晚收口，`pilot.json` 已 aggregate 三臂）

291 篇 / 8,914 gold mention 全覆盖，重复 0、陌生 0，`final_valid_accessed=false`，
`contract_sha256=1a4e857d…4252`，seed 13。

| 臂 | **MUC F1** | MUC P | MUC R | vs 主锚 `80.98472` | B³ F1 | CEAFe F1 | BLANC F1 |
|---|---:|---:|---:|---:|---:|---:|---:|
| **full** | **80.172414** | 0.792164 | 0.811518 | **−0.812** | 98.008777 | 97.652978 | 89.695288 |
| **permutation**（负控） | **79.575597** | **0.806452** | 0.785340 | −1.409 | 98.044215 | 97.625760 | **90.059346** |
| **remove_core** | **79.511770** | 0.794425 | 0.795812 | −1.473 | 97.968160 | 97.595463 | 89.505346 |

`gate`：`above_anchor=false`、`above_registered_control=false`（注册对照 `80.36759`，
⚠️ 但按 §10.5b 它在本周期**不可评定**——臂 2 训在历史采样器下，pair population 与本轮不一致）。

### 事前判据逐条核对（**两个分支都不落，不往任何一边圆**）

| 判据（跑之前写的） | 实测 | 判定 |
|---|---|---|
| `remove_core` 的 MUC precision 回到主锚 `.788430` 量级 | **`.794425`** | ✅ **达成且超额**（反超 +0.0060） |
| `full` 的 MUC F1 > `80.98472` | `80.172414` | ❌ **未达成**（−0.812） |
| 否证分支「MUC 不动」 | full +0.271 / remove_core +0.352 / permutation +0.742 | ❌ **不适用，MUC 动了** |
| 臂序 `full > remove_core > permutation` | **`full > permutation > remove_core`** | ❌ **破了** |

⇒ 与 (戊) 那次同一个形状：**事前写的两个分支都不落**，如实记为「都不落」。

### 本轮真正量出来的三件事

**1 · 先验错配是真的，而且被修掉了。** 三条臂的 MUC precision **全部反超主锚**
（`.7922` / `.8065` / `.7944` vs 锚 `.7884`），而周期 1 三条臂**全部低于**主锚
（`.7566` / `.7544` / `.7634`）。这两句各自都与主锚比，而主锚是一份**预测文件 + 冻结 evaluator**、
与机器和 backbone 无关 ⇒ **「precision 由低于锚变为高于锚」这个结论跨周期成立**。

**2 · 但缺口是守恒的——它整个挪到了 recall。**

| | 缺口大小 | 缺口落在哪 |
|---|---:|---|
| 周期 1 `full` | −1.084 | **100% precision**（recall `.8464` 反超锚 `.8325`） |
| 周期 2 `full` | **−0.812** | **100% recall**（`.8115` vs 锚 `.8325`；precision 已反超） |

净向主锚靠近 **0.272 / 1.084 = 25%**。⇒ 那 1.8 分**不是先验错配**——错配确实存在、也确实修掉了，
**但采样器只能决定缺口落在 precision 还是 recall 那一侧，不能把它消掉**。

**3 · ⚠️ 最要紧的一条：机制在周期 1 的臂序优势，不经采样器一变就没了。**

| 对比 | 周期 1 | 周期 2 | 变化 |
|---|---:|---:|---|
| `full − remove_core` | +0.74149 | +0.66064 | 基本持平 |
| `full − permutation` | +1.06782 | +0.59681 | **砍半** |
| **`remove_core − permutation`** | **+0.32633** | **−0.06383** | **翻号——负控反超了消融臂** |

BLANC 同向破：周期 1 是 `full 90.2893 > remove_core 89.6895 > permutation 89.5839`（与 MUC 同向），
周期 2 变成 **`permutation 90.0593 > full 89.6953 > remove_core 89.5053`**，负控在 BLANC 上**最高**。
⚠️ 本 split / 本指标的噪声地板**至今未量**，所以 `−0.064` 不能被当作噪声打发掉，
只能如实记为「这个顺序不再成立」。

⇒ **C5 那条「七个机制里唯一臂序正确」的结论，是在一个静默默认采样器上成立的**；
把那个采样器换成与推理一致的版本，负控就追平并在 BLANC 上反超。
**这不是对机制的补充证据，是对它的一条反证。**

**顺带**：采样器修复给**负控**的增益最大（+0.742），给机制臂最小（+0.271）
——与「这笔增益是 base 层面的先验修复、与机制无关」一致。

### 判定与去向

契约 `../phases/PHASE_C5_argument_uncertainty.md:119` 写：
「任一有效周期未超过 official joint 主锚……该周期失败；**两个有效周期后封存家族**」。
周期 1（`pilot-r2`）与周期 2（`pilot-r3`）**都已实跑且都未超过主锚**
⇒ **按契约自己的条款，role-compatibility 家族到此封存**，无需另行裁决。
（H1 不计入——它在设计阶段就被 C-11 + C-12 杀掉，从未进 pilot。）

**C5 定稿进「负结果与归因」章**，论文形态维持裁决 ④ 的
**「Ch6 承重 + 一章负结果归因（D4 + A4 + C5）」**。
⛔ 不申请第三格、不补跑第四臂、不去调判据。

### 这一轮给归因章留下的东西

1. **一个能写进论文的方法学教训**：负采样配置是训练分布的一部分，**不进契约就等于没冻结**；
   本项目的臂序结论有过一次是建立在它的默认值上的；
2. **一条可量化的守恒**：MUC 缺口在 precision 与 recall 之间转移，采样器决定落点而非大小；
3. **一条对 Ch4 的直接推论**（未验证，不得当结论）：A4 的 P `23.48` / R `50.25` 对锚的
   P `34.37` / R `32.05` 是同一形状，本轮说明把它掰回 precision 侧**未必**换来 F1。
   **Ch4 的机制缺口是 −23.72，量级差两个数量级，本轮结果不支持去动它。**

### 产物

| 项 | 值 |
|---|---|
| bundle | `gpu-4090:/data/TJK/ekg/runs/stages/C5/c5-v61-argument-uncertainty-r1/pilot-r3/` |
| 汇总 | `pilot.json`（`schema_version=ekg.c5_argument_uncertainty_pilot.v1`，三臂 aggregate 通过） |
| 日志 | `logs/c5_r3_{full,remove_core,permutation}.log` |
| checkpoint | 留在 4090（`CLAUDE.md`「训在哪就留在哪」），未回传 |
| ⚠️ 跨周期比较的限制 | 周期 1 在 **gpu-5090**、周期 2 在 **gpu-4090**，backbone pin 相同（`2c7ff1f1…`，已逐位复核）但机器不同。**与主锚的比较两边都干净**（主锚是预测文件）；**两个周期的臂值直接相减带机器分量**，上面的 Δ 只用于说方向，不作效应量 |
