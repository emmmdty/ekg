...
| **C-25R3** | **D4 Dirichlet posterior + cost-aware Bayes decision · selection holdout 门（唯一活动小步）**：每折 selection 文档按冻结 SHA 顺序前半拟合 `softmax(W log q+b)`、后半共 1,458 篇只验收；自然 posterior 以 Brier 计，决策固定 `argmax(w_k p_k)`。区别于单参数 temperature 和零参数固定解析逆，不调阈值/正则 | C-25R2 | gate Brier 至少比 gate-prevalence no-skill 低 `.0027` 且优于 raw；cost-aware causal F1 ≥`.300`；同一 posterior 的 plain vs cost-aware argmax 是单变量消融；文档级噪声 SD `.04101` 下，单侧 α=.05 的目标 power `.808`。不过不读 evaluation | R1 design brief + selection-only holdout report |
| **C-26** | **D4 predicted-causal phase contract**：把两项一致性中介、三臂唯一差异、稀有类护栏、配对 bootstrap 和停止条件冻结 | C-25R3 + formal input gate | full / unchanged full-context base / degree+direction+subtype+confidence-preserving rewiring 三臂；目标与 §25 逐项一致；不读方法结果后改门 | 新 D4 phase contract + preflight |
