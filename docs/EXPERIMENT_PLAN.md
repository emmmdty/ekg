...
| **C-25R** | **D4 posterior 校准修复（唯一活动小步；首轮方法已冻结）**：每折用该折 selection-dev 的 exhaustive posterior 与 unweighted multiclass NLL 拟合唯一标量 `T>0`，evaluation 只应用 `softmax(log(p)/T)`；不选方法、不调类别阈值。Guo et al. 2017 正文与官方代码均核实为固定模型、validation-only 的单温度校准 | C-25 | 原始 sidecar 不覆盖；evaluation label 不参与拟合；argmax/prediction counts/causal F1 与 raw **精确不变**；Brier `< .0414265581`。失败则保留结果并立项会改变 argmax 的 class-bias 第二周期，禁止在 evaluation 上改 `T` | selection posterior + calibration metadata + calibrated sidecars + quality report |
