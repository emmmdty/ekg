...
| **C-25R** | ✅ **D4 scalar-temperature 校准周期完成，正式判失败**：五折 `T=2.0857/1.8881/2.1734/1.7064/.7514`，selection NLL 全降且 evaluation argmax/F1 `.308141` 精确不变；正式 Brier `.068750 → .064155`，但仍劣于 no-skill `.041427`。报告 SHA `5fc28b7c…7b296`，原始与 calibrated sidecar 均封存 | C-25 | 覆盖/F1/argmax PASS；Brier FAIL。证明全局温度只解释约 6.7% Brier，相同 family 不再扫 `T` | R1 calibrated quality report |
| **C-25R2** | ✅ **D4 class-weight loss correction 在 selection-only 门正式失败**：解析逆使 pooled Brier `.068231 → .041942`，但仍未严格优于 no-skill `.041427`；pooled causal F1 `.308297 → .279911`，低于 `.300`。fold 5 正类预测从 24,895 降至 1,242，是跨折损伤的主要异常。报告 SHA `fb887785…fcd6f8` | C-25R | `evaluation_artifacts_accessed=false`；三项门仅 raw-Brier improvement 通过，F1/no-skill Brier 失败；不生成 evaluation sidecar，不插值校正强度 | R1 selection audit |
| **C-25R3** | ✅ **D4 Dirichlet posterior + cost-aware Bayes decision 通过 selection document holdout**：1,455 篇拟合、互斥 1,458 篇 gate；自然 Brier `.034240`，比 no-skill `.040057` 低 `.005817`（目标 `.0027`）；cost-aware F1 `.300990` 过线，plain argmax `.081921` 的单变量消融确认决策分离必要。报告 SHA `ee7835d0…cad90f` | C-25R2 | 三项 gate 全 PASS，`evaluation_artifacts_accessed=false`；实现 `21d9824`，本地 727 passed、远端 17/17 passed | R1 Dirichlet holdout audit |
...
| **C-28** | **用官方代码把「加关系涨 2 分」这条已发表结论在 gold 与 predicted 两种结构下各跑一次**（`THU-KEG/MAVEN-FACT` `6754471` 的 `trainEFD/train.py --add_relation`，零新机制代码）。**科研价值（C-28a 之后更锋利）**：oracle 已排除「结构本身」这一半——我们的消息传的正是前驱 mention 的编码表示，给金标图仍是零增益。于是剩下的问题很具体：**原论文的 `+2` 到底来自前驱句的文本，还是来自关系这条结构？** 这一条要回答的是「**领域那条结论本身**在预测关系下还成不成立」——若 gold 行复现出 `+2` 而 predicted 行归零，负结果归因章就从「我们没做成」升级为「这条路在部署条件下不成立」，对准表 3-3 与表 3-5。**可行性**：代码可运行、数据（我们的 MAVEN-FACT + 已冻结五折 sidecar）齐备、4090 空闲、无需授权。⚠️ 两处必须透明披露的适配：官方脚本在 **test 上选 epoch**（`train.py:207`，改为 dev 选模）；官方数字所在的 test 划分拿不到（FR-016 只能到 **(b)**，见 `BASELINE_ROSTER.md` §3） | G-18 判定完成 ✅ | gold 行与 predicted 行**只差结构来源**，其余逐位一致；不调它的超参、不换映射；结果只进 `results/PHASE_R1.md` 与表 3-3/3-5 | 官方代码两行 + 适配差异清单 |
...
| MAVEN-FACT 官方 EFD pipeline[Li+ 2024] | 待测 | 待测 | 待测 | 待测 | 待测 | 待测 | 待测 |
| LLM 对照（Qwen3-8B LoRA） | 待测 | 待测 | 待测 | 待测 | 待测 | 待测 | 待测 |
| ~~本文 D4 机制一（typed cue + 分解式决策）~~ **已封存 / 失败** | — | — | — | — | — | **.4765** | — |
| ~~本文 D4 机制二（predicted-causal uncertainty-gated residual）~~ **已封存 / 失败** | .9732 | .5341 | .6699 | .3501 | .1760 | **.5407** | .9448 |
...
⚠️ **本章现状：两个机制家族都已封存，第 3 章按负结果章写。** 上表两条「本文」行的数字只从
`results/PHASE_D.md`（机制一）与 `results/PHASE_R1.md` §25.20（机制二）引用，不在本文件重算；
两个家族都**低于**主锚 `.5540`，所以本章**不主张任何方法优势**。

**表 3-4 消融与负控（机制二，seed 13 五折 OOF；机制一的三臂见 `results/PHASE_D.md`）**

| 臂 | 5类 macro-F1 | PS− | Uu | vs base（配对 95% CI）|
|---|---:|---:|---:|---|
| full（不确定性门控因果残差） | .5407 | .3501 | .1760 | `−.002495`，CI `[−.0156, +.0105]` |
| base（不接结构信息，单变量对照） | **.5432** | .3616 | .1866 | — |
| rewired（保度·方向·子类·置信度多重集的重连，**负控**） | .5326 | .3642 | .1399 | full−rewired `+.008080`，CI `[−.0051, +.0215]` |

同一三臂在 G-18b（只换打包规则）下的敏感性观测，**不替代上表**（§25.23）：
`full .5499` / `base .5431` / `rewired .5333`；`full−base +.006740` CI 跨 0，
`full−rewired +.016531` 下界 `+.000115`。两跑的一致性中介**都**是反向的。

机制成立的形态本应是 `full > base` 且 `full > rewired` 且两项一致性中介下降。
**实测三条全不成立**（`full` 反而低于 `base`，两个 CI 都跨 0，中介一项持平一项更差）⇒ 机制二证伪。

**表 3-5 负结果归因（本章真正要交的表）**

| 证据 | 数字 | 出处 |
|---|---|---|
| **给完美的图也没有增益（本章最强的一条）** | oracle（53,358 条金标边）pooled `.542915`，`oracle − base = −.000223`，CI `[−.0147, +.0143]` | §25.25 |
| 输入概率经三次修复才可用 | Brier `.068750 → .064155 → .041942(拒) → .035135`，no-skill `.041427` | §25.9–§25.13 |
| 边的精确率 | **`.2243`**（114,479 判正边 / 25,681 条对） | §25.21 |
| 精确率的天花板 | 置信度过滤最多买到 `.50`，那时触及面只剩 **3.2%** | §25.22 |
| 机制的触及面 | 47.8% 的 mention 有入边，只有 18.3% 有**正确**入边 | §25.21 |
| ~~失败形态＝稀释而非方向错~~ **已被 §25.25 推翻** | 事后分层曾见入度 ≥2 的 22,069 个 mention 上 `full .5031 > rewired .4835 > base .4761`，但 oracle 证明边质量不是主因 ⇒ 该解释作废，这一层只作未解释现象记录 | §25.21 → §25.25 |
| 一致性中介事前就没有分辨力 | gold 地板 `.003602 / .007744`，三臂 `.0058 / .0091` | §25.16 |
| 负控偏保守 | 重连后 **40.53%** 的边仍是同一 `(head,tail,subtype)`，预算抬到 200× 也只 41.27% | §25.16 |
| **配置敏感度 > 效应量**（本章最该写的方法学结论） | 只换打包规则，`full` 动 **`+.009207`**（`.540671 → .549878`），而更好那一跑的 `full − base` 只有 `+.006740` | §25.23 |
| base 欠额与优化步数无关 | 910 步与 1,248 步下 base 是 `.543166` 与 `.543138`，**差 `.000028`** | §25.23 |
