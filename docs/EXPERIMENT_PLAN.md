...
| **C-28a** | **gold 边 oracle 诊断行（non-deployable）**：把同一条流水线喂一张**完美的**因果图，看 §25.21 的「边只有 `.2243` 对」是不是失败的主因。零机制代码改动——只把冻结 sidecar 的三类概率按金标逐行改写（`build_d4_gold_sidecars.py`），文档/候选对/顺序全部原样。**科研价值**：把「边太脏」与「机制本身不起作用」分开，这是负结果归因章缺的最后一块。**可行性**：数据、代码、4090 均具备，5 个 run 约 1 小时 | C-27 ✅ | 契约给 oracle **不设臂**：只作结果页诊断行，**不得进任何通过门**；oracle 的「边对」与「置信度高」按构造一起变，写作时必须说明不是单变量 | `.../seed-13-oracle/`（`results/PHASE_R1.md` §25.25）|
