...
| **C-6** | A4.0 实现 + 本地 gate | 无 | **2026-09-12 完成核心件**（`790c35a`）：`src/ekg/relations/pair_evidence.py`（每对证据集 / masked+retained 反事实句集 / 注册中介 unsupported 跨句 causal 误报 + 分母 / per-pair sidecar / 臂身份与 selector 哈希漂移拒绝）＋ `pair_heads.py` 注册 `pair_evidence` 头（零初始化证据残差，四臂同参数、init 基础 logits 相同）；**20 条 targeted tests**，本地 **578 passed / 28 skipped、ruff 0、smoke OK**，两条 torch 门在 gpu-5090 实跑通过（本地 skip 不算数）。选择器**确定性、无阈值可扫**：按 `crosssentence.py` 冻结线索词表 + 触发词复现 + 与触发句距离排序，取 `EVIDENCE_BUDGET=2` 句；学习部分只在 encoder 与残差。bundle exporter 复用 `create_stage_bundle`，不另写。**C-6b 亦于 2026-09-12 完成**（`65cf64b` + `0615596` + `6a5fabe`）：`train_` / `evaluate_` / `prepare_*_preflight` / `smoke_` / `run_a4_pair_evidence.py` 五个入口齐全，pilot 入口已进 preflight 的 `CODE_FILES`（7 个文件）；**44 条 targeted tests**，本地 **592 passed / 28 skipped、ruff 0、smoke OK**。5090 上跑完四臂开发冒烟（1 epoch / 10 篇 / 公开 roberta-base，**非结果**）：四臂候选逐键相同、反事实 logits 有限；冒烟连抓三个真缺陷并修掉（TIMEX 对进 causal 监督 → 全 ignore 的 CE 返回 nan；训练用的 cap 被用在推理上 → 预测取决于候选顺序且中介分母为空；第二段训练分布 80% 是 NONE → 把 causal 正类全部抹光）。**选择器设计已按本项目自己的测量否掉重做**：连接词词表证伪（.008/.064，且把 79.3% 的对判成有线索）→ 证据改为「两触发句之间的 interior」定义式。preflight 本地已验到 relation contract 与**复算出候选 digest `15a3b1a5…dac10910`**；**待 4090**：encoder 内容摘要与两条 baseline 官方预测。详见 `results/PHASE_A.md` | 代码 + 测试 |
