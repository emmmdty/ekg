...
| **G-18b** | **D4 五折三臂的执行修复重跑**：只把按文档打包的切分规则由「累加到 ≥32 才切」改为「取离 32 最近的边界」（910 → 1,248 步/epoch，48.8 → 35.6 mention/步，锚是 32 / 1,387）。起因是 G-18 fold 1 的 `base` `.514779` 比同折冻结锚 `.5568315` 低 `.042`，逐 manifest 重算确认优化步数少三分之一（`results/PHASE_R1.md` §25.19）| G-18 跑完 | 与 G-18 同量级 | **同一周期的执行修复，不是第二个机制周期**；不动门常数/阈值/backbone/seed；两跑都进结果页。若修复后 base 仍显著低于锚，则欠额另有来源，按 §10 处理，**不再第三次调 batch** |
