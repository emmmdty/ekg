...
| **C-28** | 🟡 **在跑（2026-09-22 07:46 启动，4090 卡 0/1/2，10 个 run）**：官方代码已 vendored 到 `baselines/maven_fact/`（上游 `6754471`，原样 hash 在 `UPSTREAM.json`），**只打一个补丁**——上游在 test 上选 epoch，改为必填 `--dev_data` 按 dev 选并同时报上游那个逐轮最大值。两行只差 `causal_relation` 的来源（gold / 我们的 predicted 提升到簇级）。1 epoch 冒烟已通过。⚠️ 官方用无类别加权 CE，稀有类与我们的锚本就不可比；backbone 用 roberta-base pin 而非它默认的 roberta-large，偏离已记 | G-18 判定完成 ✅ | 约 7 GPU·h | FR-016 只能到 **(b)**；不调它的超参、不换映射（`results/PHASE_R1.md` §25.26）|
