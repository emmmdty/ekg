...
| **C-25R3F** | **D4 Dirichlet formal input gate（唯一活动小步）**：每折在完整 selection 上按同一无正则 Eq.7 refit，先封存五份 natural-posterior sidecar/metadata，再读取 evaluation gold 一次；硬判定只用同折冻结训练权重的 `argmax(w_k p_k)` | C-25R3 | 2,913 docs / 2,532,394 pairs 精确覆盖；formal natural Brier ≤`.0387265581`（比 no-skill 至少低 `.0027`）且优于 raw；cost-aware F1 ≥`.300`；同时报告 plain argmax 消融。任一失败即封存，不再改 map/阈值 | five sidecars + formal quality report |
| **C-26** | **D4 predicted-causal phase contract**：把两项一致性中介、三臂唯一差异、稀有类护栏、配对 bootstrap 和停止条件冻结 | C-25R3F pass | full / unchanged full-context base / degree+direction+subtype+confidence-preserving rewiring 三臂；目标与 §25 逐项一致；不读方法结果后改门 | 新 D4 phase contract + preflight |
