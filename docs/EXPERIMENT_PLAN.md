...
| **C-25R2** | **D4 class-weight loss correction · selection-only 可行性门（唯一活动小步）**：按训练器实际 class counts/`alpha=.5` 重算每折 `w_k`，只在 selection posterior 应用解析逆 `p_k=(q_k/w_k)/Σ(q/w)`。依据 Caplin et al. 2022 的 weighted-loss miscalibration 与 invertible loss correction；无拟合参数，不读 evaluation | C-25R | pooled selection exact-subtype F1 ≥`.300`，且 corrected Brier 同时低于 raw 与 selection-prevalence no-skill；不过则停在 R1，不生成第二套 evaluation sidecar。通过后另行冻结正式一次性评测 | selection-only feasibility report |
| **C-26** | **D4 predicted-causal phase contract**：把两项一致性中介、三臂唯一差异、稀有类护栏、配对 bootstrap 和停止条件冻结 | C-25R2 + formal input gate | full / unchanged full-context base / degree+direction+subtype+confidence-preserving rewiring 三臂；目标与 §25 逐项一致；不读方法结果后改门 | 新 D4 phase contract + preflight |
...
作者已把活动队列改为 **D4 单章：C-23 → C-24 → G-15 → G-16 → C-25 → C-25R → C-25R2 → C-26 → C-27 →
