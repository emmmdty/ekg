...
| **C-25R** | **D4 posterior 校准修复（唯一活动小步；实现与 selection 输入已完成）**：每折用该折 selection-dev 的 exhaustive posterior 与 unweighted multiclass NLL 拟合唯一标量 `T>0`，evaluation 只应用 `softmax(log(p)/T)`；不选方法、不调类别阈值。Guo et al. 2017 正文与官方代码均核实为固定模型、validation-only 的单温度校准。五折 selection posterior 已封存，校准/汇总代码提交 `078fe00`，本地完整门 715 passed | C-25 | 原始 sidecar 不覆盖；evaluation label 不参与拟合；argmax/prediction counts/causal F1 与 raw **精确不变**；Brier `< .0414265581`。当前待 4090 隧道恢复后同步 commit 并执行；失败则进入 class-bias 第二周期 | selection posterior + calibration metadata + calibrated sidecars + quality report |
