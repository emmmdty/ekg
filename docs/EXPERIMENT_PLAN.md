...
**协议障碍的原文**：`phases/PHASE_C5_argument_uncertainty.md:93` 写「不扫 threshold/epoch」，
`:121` 写「不以更大 backbone、threshold/epoch sweep、换 split 或 oracle argument 救结果」。

- 改 `--neg-ratio` **不是 threshold**，它是训练数据构造；但它动的是被契约钉住的训练配置
  ⇒ 必须**重建 preflight**（§0.7 纪律 6，旧的不覆盖），且要事前登记判据；
- **推理侧的解析先验修正**（按已知采样比平移决策函数，单值由采样设计决定、不从结果里挑）
  在字面上贴着 `:93` 那条红线 ⇒ **单独裁决，不与上一条打包**。
...
| 步 | 内容 | 成本 | 事前判据 |
|---|---|---|---|
| **步 1（建议先做）** | C5 在 **4090** 重跑 `full` + `remove_core` 两臂，**唯一变量 = 负例分布**（`--neg-ratio` 提到全量或按设计值），重建 preflight | 2–4 h，4090 自用 | `remove_core` 的 precision 是否回到锚的 `.7884` 量级；`full` 是否 > `80.98472`。**MUC 不动 ⇒ 共因假设否证**，C5 就此定稿进负结果章 |
