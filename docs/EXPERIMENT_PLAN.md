...
| **G-11** | Ch6：E3.0 → E3.1 → E3.2 → E3.3 → E3.4 → E3.5。⚠️ **E3.1 的前置 2026-09-17 收紧**：C5/D4 的 `fallback_component_bundle_id` **补不回去**（两个 runner 都在各自契约的 `code` 哈希集合里，pilot 又已收口 ⇒ 改代码必重建 preflight，而各臂钉的是旧契约、`aggregate` 会拒绝；只能重训才落地）⇒ **E3.1 必须自带 fallback 登记步骤**，不得指望从 phase summary 统一读到该字段。三份 fallback 的候选身份与 content hash 已查好（identity=official_joint `66ff04ba…`、relation=A4 summary 里已有、factuality=`cls` OOF `f375e8a5…`），见 `results/PHASE_E.md` | C-4 + 至少一章的 bundle/fallback + G-0 | ~1–2 GPU·day | 见 `phases/PHASE_E3_graph_application.md` 的 Done when |
...
| **下行** | ≤1 个方法章过门 | **由作者裁决改纲**（导师不介入结构，只看结果）。Ch6 的构建损失、图依赖正控与不对齐发现**不依赖任何方法章成功**，仍然成立 |
