...
| **C-27** | **D4 uncertainty-gated residual 实现与本地门（唯一活动小步）** | C-26 ✅ | 无边/低置信度时残差严格趋零；三臂参数与预算一致；ID/schema/置换/梯度/聚合测试通过，三件套全绿 | code + tests + CPU smoke |
| **C-27b** | **D4 训练文档的结构输入 dump**：用每折**自己的** causal checkpoint 对该折 `train.json` dump raw posterior（1,747/1,747/1,748/1,749/1,748 篇，合计 **7,597,182** ordered pairs），再用同一张冻结 fold map 做 Dirichlet 变换。**起因是 C-27 开工自审查出的泄漏路径**：OOF posterior 并集会让 fold i 的 evaluation 金标关系经 relation fold j 间接流进训练（分析见 `results/PHASE_R1.md` §25.14） | C-26 ✅ | 不训练模型、不读 evaluation、不改 map；train 侧 in-sample 乐观如实披露（该偏置**偏离**我们的结论）；沿用 `dump_relation_causal_posteriors.py` + `formalize_d4_dirichlet_posteriors.py` 原样调用 | 五折 train posterior + Dirichlet sidecar |
