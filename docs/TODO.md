...
1. **⭐ 队首 = Phase A2：按官方配方重训 Ch2 抽取器**
   ...
   **⇒ 第二个根因（2026-08-07 续）：一直漏配官方的 warmup + linear decay**
   （`--warmup-steps` default 0，scheduler 只在 >0 时创建）——所有既有档都是恒定 lr。
   对照实验：无 warmup 那跑峰值卡在 epoch 0、20 轮无一超越；补上后 ep9 起持续爬升。
   **⇒ 第三步：选择信号被样本量绑架**——best-epoch 用三族合并 micro F1，而 temporal 候选对是
   subevent 的 39 倍，micro 值几乎等于 temporal 单独值 ⇒ 改 `--dev-metric macro` 后
   subevent 19.65→**21.05**、temporal 让出 0.88，方向与预测一致。
   **终点 `window_dist_20ep_macro`：causal 28.50 / temporal 31.55 / subevent 21.05**，
   与官方原版同 valid 的 31.37 差距 **7.46→2.87**。
   ⇒ **下一步（按证据排序）**：
   ① **跨句仍是主瓶颈**：25.00 vs 同句 38.18，且它只被窗口编码推动过一次；
   ② ⚠️ **subevent 21.05 仍低于起点 24.03**（macro 只是部分修复，不是已解决）；
   ③ **梯度累积已实现未验证**（官方 batch 8 / 我们逐文档=batch 1，且这让 warmup 200 只等于
      200 篇而非官方的 1600 篇）；④ 事件类型 embedding（⚠️ 词表须随 checkpoint 存）；
   ⑤ **多种子**——距离流(+0.65/−0.71)等小效应至今分不清噪声。
