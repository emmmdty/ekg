...
## 下一步：**主表 C-11 · H1 离线验证**（C5 第二设计周期第 1 步）

`full` 的 419 个误合并里 trigger 相似度 ≥0.8 的跨句桶占 **58.0%**（主锚 51.5%）。C-11 要出两件：
①该桶是否真的是误合并的集中处；②**抑制上界**——按不同阈值拒绝该桶后 `full` 的 MUC P/R/F1 各变多少
（对照 .756630 / .846422 / 79.90115，主锚 .788430 / .832461 / 80.98472）。
纯离线、复用 G-5b 产物、**不训练不占卡**。上界够不到主锚 ⇒ 抑制侧接法不够，换接法而非调阈值。
