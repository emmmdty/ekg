...
| E | Ch4 三图误差传播与归因 | 🟡 SeDGPL/受控扫描已有；三图分解未做。**门槛已按 2026-07-29 重定位改写** | **三图误差曲线 + 下游损失可归因到具体构建/修复动作**；净化下游增益正面回答。~~repaired > predicted~~ **已作废**（实测门控天花板 0.24%，不再作为门槛） |
...
4. **⭐当前队首 = Phase E（Ch4 三图误差传播与归因）**，A·B·C·D 已齐；两个口径已于 2026-07-29 拍板。
   ...
   **归因到具体的构建与修复动作**（已有的归因样板：1.4 万次 temporal 编辑按构造零影响、
   858 次 causal 破环 → −0.24%）。这是 headline 的正文。
   (b) **净化的下游判定** —— 正面回答"事实性净化对后继预测有无增益"，不绕开；
   若无增益，Ch3 按止损口径退为「检测 + 预测图鲁棒性分析」。
   (c) ⚠️ **若仍要保留任何门控实验档**，门控信号须先满足 SPEC §4.5（禁金标 MRR），
   且门控**只对 causal 环破除有意义**（其余修复动作对下游按构造零影响，门控它们是空转）。
   预测边 dump 已在 4090 `runs/factuality/predicted_edges_valid.jsonl`，E 直接复用不必重算。
   ⚠️ **先验预期要放低**：M1/M2 的 A/B 与修复归因都落在噪声级（ΔMRR ≈ ±0.002），
   **在 SeDGPL 这个下游上图侧干预普遍只有噪声级效应** —— 别再用新机制撞同一堵墙，
   把它当作要**解释**的现象。
