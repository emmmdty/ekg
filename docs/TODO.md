...
1. **第 6 章外部对手复现**（G-11a）：SimKGC / BART contrastive / MCPredictor 各自取 (a) 或写明障碍维持 (b)。
   5090 已空，这条线不依赖任何方法章；
2. **C5 差距归因**（G-5b，离线、不训练、不占卡）：`remove_core` 79.15966 比主锚低 **1.83**
   ⇒ 差距主体在共指主干而不在机制。把这 1.83 拆到漏合并 / 误合并 / 单例判定上，写回 `results/PHASE_C.md`。
   **这不是第二设计周期**，是给改纲裁决备证据；
3. **LLMERE 恢复方案审查**（C-3，纯文档只读，主表排 09-17 起 3 天）；
4. **A4 误差分析**（G-4b）：四臂召回坍塌砍掉了哪些 causal 类型、砍的是不是难例，写回 `results/PHASE_A.md`。

已完成并已推送：A4.3 收口（09-17）、C5.3 与 CSProm-KG 收口（09-16）、本周周报
`reports/2026-09-17_周报.md`。
