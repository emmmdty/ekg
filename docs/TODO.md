...
- **关键路径进展**：Phase A ✅ 达标（causal F1 .250，召回 .4%→67.5%）→ Phase B 🟡 违反清零但下游无增益
  （止损）→ Phase C 🟢 MUC 79.6 vs 官方 81.4 基本达标 → **Phase D 🟡 检测达标（valid macro-F1 .4899）、
  净化止损**（2026-07-28）。**A·B·C·D 已齐，关键路径转 Phase E（Ch4 闭环 headline）。**
- **执行状态**：P0 数据完成；A/B/C/D 均已跑出真实数字（见下各节，升降如实）。
  ⚠️ **进 Phase E 前有两个未决口径必须先跟作者拍板**：① Phase B 已证「修复提升下游可重建性」在真实图上
  不成立，而 SPEC 的 headline 正是 Ch4 下游门控闭环修复 —— 须重新界定该主张，不得在 E 里换指标掩盖；
  ② Phase D 的净化在结构一致性上是负结果，其价值改由 E 的后继预测判定，E 必须**正面回答**它，
  而不是绕开。
...
| 系统 | macro-F1 | accuracy |
|---|---|---|
| 全预测 CT+ 平凡基线 | .1947 | **.9487** |
| lexicon 记忆基线 | .2233 | .9409 |
| no-structure 消融 | .4743 | .9482 |
| **本系统（gold 图输入）** | **.4899** | .9456 |

- 每类 P/R/F1：CT+ .9851/.9634/**.9741**｜PS+ .4185/.5965/**.4919**｜CT− .5250/.7109/**.6040**｜
  PS− .1410/.2115/**.1692**（n=52）｜Uu .1622/.3000/**.2105**（n=20）。
- **accuracy 无信息量的实证**：lexicon 的 accuracy（.9409）**低于**平凡基线（.9487），macro-F1 却更高。
- evidence：**macro(CT−/PS+/PS−) .6042**（CT− .6392 / PS+ .5016 / PS− .6719），pooled F1 .3639
  （P .2438 R .7174，n=1,288 spans）。同句候选的**召回上界 .974** 已显式命名，未触顶。
- **结构特征贡献 +.0156**（.4899 vs .4743），集中在小类：PS− +.058、Uu +.053，CT+ 上 −.001。
  ⚠️ 该对比对结构档**不利**（消融用了自己的 dev 最佳 ep4，主档用的是最后 ep6 而非其最佳 ep5）。
- ⚠️ dev macro-F1 **非单调**（.5077/.5321/.4918/.5325/.5916/.5491），由 PS−(~28 例)/Uu(~12 例) 方差主导；
  故训练改为**保留 dev 最佳 epoch**并落 `dev_curve.json`。**单次运行，强结论需多种子（Phase H）。**
...
- 把 gold 图（126,486 边）换成 **Phase A 抽取器的预测图（231,530 边，1.8 倍）**，同一检测器、
  只改这一个输入：macro-F1 **.4899 → .4898（−.00015）**，17,780 个 mention 中**仅 9 个**标签改变。
- 解读（两条不矛盾）：结构特征用的是**粗粒度度数统计**，gold 与 predicted 图的度数分布量级相近，
  经 log1p 压缩后特征位移极小，不足以翻转 argmax；而消融是把这 8 维**整个去掉**，差异自然更大。
- 对下游是**正面**结论：构建阶段的图错误**不传染**到事实性判定。但同时说明"结构感知"是
  **弱机制**，不宜作为卖点，**不得包装成"我们的方法鲁棒"而回避这一点**。

**净化 —— ❌ 负结果，止损触发**：

预测图上按 CT− 剔节点 / PS−·Uu 降权：节点 17,780→17,261（剔 519）、边 231,530→222,134
（剔 9,396、降权 979）。**加随机剔除同样数量节点的对照后，净化在每一项一致性指标上都不如随机**：

| 指标 | 净化前 | 净化后 | 随机剔 519 节点 |
|---|---|---|---|
| n_edges | 231,530 | 222,134 | 218,602 |
| causal_cyclic_scc | 937 | 909 | **888.8** |
| temporal_cyclic_edges | 42,980 | 41,568 | **39,779** |
| temporal_closure_gap | .3721 | .3691 | **.3648** |

- 归因（可解释）：**CT− 事件是低度数节点** —— 剔 519 个只带走 18.1 边/节点，随机则是 24.9 边/节点。
...
产物：`runs/factuality_valid_6ep.json`、`runs/factuality_valid_nostruct.json`（已取回本地）；
...
| D | Ch3 事实性与净化 | 🟡 **检测达标、净化止损**（2026-07-28，4090）：valid macro-F1 **.4899**（平凡 .1947 / lexicon .2233 / 官方 DMBERT 47.6@**test**）✅；evidence macro(3类) **.6042**（官方 45.4）✅；**预测图掉点 −.0001**（9/17,780 标签变）—— 鲁棒但说明结构是弱机制 ⚠️；**净化不如随机剔同样节点数 ❌ 止损** | macro-F1 ✅、预测图掉点 ✅、净化下游增益 ❌（转 Phase E 判定） |
...
3. **Phase D 主结果已出**（2026-07-28，4090 card 2；见上文「Phase D 实施」）。**收口三件事**：
   ① **检测达标**：valid macro-F1 **.4899** vs 平凡 .1947 / lexicon .2233 / 官方 DMBERT 47.6，
   evidence macro(3类) **.6042** vs 官方 45.4 —— 但**官方数字在 test、我们在 valid，属不同 split**，
   写作时必须显式声明，不能简写成"超过 47.6"；
   ② **鲁棒性 = 几乎不掉点**（−.0001，9/17,780 标签变）：对下游是好消息（构建误差不传染），
   但同时说明"结构感知"是**弱机制**（消融只值 +.0156，且集中在 PS−/Uu 两个小类）——
   **不得包装成"我们的方法鲁棒"而回避机制弱这一面**；
   ③ **净化 = 负结果**：加随机剔除对照后，净化在每项一致性指标上都不如随机剔同样数量节点
   （因 CT− 本就是低度数节点）。**结构一致性这条路已封**，价值改由 Phase E 的后继预测判定。
4. **⭐当前队首 = Phase E（Ch4 闭环与三图传播）**，A·B·C·D 已齐。
   ⚠️ **开跑前必须先跟作者拍板两个口径**（都不是 E 内部能自行决定的）：
   (a) **Ch4 headline 重定位** —— Phase B 已证「修复提升下游可重建性」在真实图上不成立，
   而 SPEC 的 headline 正是下游门控闭环修复；不得在 E 里换指标掩盖该负结果；
   (b) **Phase D 净化的归宿** —— E 必须正面回答"事实性净化对后继预测有无增益"，
   而不是绕开；若同样无增益，则 Ch3 按原止损口径退为「检测 + 预测图鲁棒性分析」独立成章。
   预测边 dump 已在 4090 `runs/factuality/predicted_edges_valid.jsonl`，E 直接复用不必重算。
...
7. 实现 Ch4 闭环控制器前，**先定死门控信号来源**（SPEC §4.5：无标签代理 or 离线诊断定位），别默认用金标 MRR。
