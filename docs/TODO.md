...
3. 🟢 **A4.3 四臂在 5090 上跑**（作者 09-16：「4090 不可行就使用 5090」），契约 `11e4343a…490c`，
   约 4–5 小时 → `--aggregate` → 写 `results/PHASE_A.md`。**Gate 2 唯一还缺的输入**。
   ⚠️ 读数带机器偏移限制：**四臂互比干净**，与 33.17 / 32.10 / 32.01 的比较跨机器（约 1 点），
   **只陈述、不相减**；4090 线的 `preflight-r2 a3cc6c44…f73c` 保留，将来空卡可补跑；
4. Gate 2：等 A4.3 出结果再判，执行代理不得提前宣布章节存废。
