...
> **FROZEN / NOT STARTED。** 本契约由 R1 T023/T024 放行；只允许先完成实现、本地 gate、baseline
> preflight 和 CUDA smoke。额外 seeds 未获授权，seed-13 pilot 过门前不得建立 seed-17/42 目录。

## Goal

检验 independently predicted mention-local participant/place 精确 spans，以及由这些 hard spans 学得的
pairwise role-compatibility posterior 与缺失不确定性，能否在完整 MAVEN-ERE mention candidate 上减少角色
冲突的高置信 false merge，并在官方 MUC 与全部 coreference 护栏上超过 official joint 和同协议
argument-aware baseline。Qwen3 本身不提供已校准 role posterior；不得把生成字符串伪称为概率。
...
2. 退火 local pair classifier（no-argument reproduction base）；
3. Qwen3 mention-local participant/place + learned argument-span pair pooling（argument-aware strong baseline）；
...
6. hard arguments without uncertainty；
7. document×event-type 内 role posterior permutation（negative control）；
8. MAVEN-ARG event-level gold arguments 仅列 non-deployable oracle，不参与胜出门。

除注册组件外，2–7 的 encoder、pair population、optimizer steps、seed、预算和 scorer 必须逐位一致。
...
物化 `runs/stages/C5/c5-v61-argument-uncertainty-r1/preflight/`，冻结 source/manifest/candidate/evaluator、
extractor、encoder、code/config/command hashes 与 final-valid ledger。重算三条 deployable baseline 的 official
MUC/B3/CEAFe/BLANC；任何 population 或 hash 漂移立即停止。
...
只运行矩阵 3–7 的 seed 13。训练期仅读 train；internal-dev 只按固定退火终点评分，不扫 threshold/epoch。
...
- seed-13：full 的 MUC 严格高于 official joint 与 argument-aware baseline；full 相对 remove-core 降低注册的
  incompatible-role false-merge rate，且 permutation 消除该中介改善；
