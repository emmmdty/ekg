...
- ⛔ **启动类命令不要套 `for … && break` 重试循环**（2026-09-16 连犯两次）：后台启动的 ssh
  **本来就返回非零码**，重试循环会把同一条命令再发一次。第二条通常被产物守卫挡住
  （「refusing to overwrite」——守卫是对的），但**它的 stderr 会把第一条的日志截断**。
  **重发前先核一次产物目录与 `ps`：ssh 失败 ≠ 命令没执行。**
- ⚠️ **别拿一个臂的耗时去估另一个臂**：A4 的 `remove_core` 50 epoch 是 56 分钟，而带证据流的
  `full` 是 **4.2 小时**（多做反事实前向）。2026-09-16 因此把 A4.3 报成「4–5 小时」，实际 10–13 小时。
