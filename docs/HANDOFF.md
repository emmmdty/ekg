...
| **活动任务** | **无。两台机上都没有我们的进程。** 新窗口从 §0.3 的队列开工，队首 **C-5b**。 |
| **Ch4（A4）现状** | ✅ **方法与五个入口脚本齐全**（C-6 + C-6b，2026-09-12/13），5090 四臂开发冒烟通过，设计已按证据自审改过一轮并核掉 DREEAM 替代方案。**只差 A4.1 preflight**，它卡在 4090 上的两个文件（见 §0.4）。 |
| **待作者裁决（1 项）** | **A4 决策 3：推理时要不要两段式修正。** 现状=两段式（已实现、已冒烟）。另一选项=机制只作训练目标、四臂推理口径完全一致。**它会改动冻结矩阵第 8 臂的含义，执行代理不得自行改**；详见 §0.5。 |
| ⚠️ **gpu-4090** | 驱动已修好（580.178.04，G-0 完成），但**四张卡被用户 `Zhyw` 的 vllm 占满**——**2026-09-13 复核仍然占着**（各约 18.6–20.5 GB / 24.5 GB）。**再用必须先 `nvidia-smi` 核卡，不得挤占**；空出来的第一件事是 A4.1 → G-4。 |
...
**GPU 泳道**：
- **G-4（A4.2 smoke → A4.3 pilot）不依赖上面任何一行**，只等 ① 4090 空出卡 ② A4.1 preflight 跑通
  （所需的两个文件见 §0.4）③ 作者授权长任务。**4090 一空就该先排它**——A4 是唯一实现完成、
  入口齐全、冒烟过的一章。
- **G-5（C5.2 smoke → C5.3 pilot）** 等 C-5b + 队列第 2 行。
- 两者**写不同 namespace，可并卡**。跑完这两个就到 **Gate 2**。

### 0.4 Ch4（A4）现状：设计、缺陷、以及 A4.1 还差什么
...
### 0.5 待作者裁决：A4 决策 3（**执行代理不得自行改**）

**推理时机制动不动手。** 两个选项的区别只有一条，但它改的是被评分的规则：

| | (A) 两段式修正 —— **现状，已实现已冒烟** | (B) 只作训练目标 |
|---|---|---|
| 推理 | base 判正的行用证据残差重打分，改写标签 | 四臂推理口径**完全一致**；反事实只作测量 |
| 冻结矩阵第 8 臂 | 原样（「证据表示保留、约束关闭」） | 含义改变——**动了冻结矩阵，所以必须作者点头** |
| 机制的作用通道 | 两条（encoder + 修正头） | 一条（只有 encoder），**出空结果概率更高** |
| 归因 | 由第 8 臂 `no_constraint` 分离「修正头的功劳」与「一致性的功劳」 | 天然无此问题 |

**未收到裁决就按 (A) 继续**，A4.1 不因此阻塞。**要改就必须在 pilot 出数字之前改**——
一旦 A4.3 跑出来再换设计，那一轮直接作废。
