...
> 更新于 **2026-09-17（收工整理）**。本文是新窗口唯一必读入口；读完后再按本文链接打开所需文件，
> 不回溯聊天记录。本文只记录**状态、决策、依赖、下一步**，不复制实验表格——
> 数字只认 [`results/`](results/README.md)。
>
> **现在的状态一句话：三章的第一个设计周期全部跑完、0 章过门，队列已空，
> 三项待作者裁决（§0.3）。在拿到裁决之前，执行代理没有可自行推进的任务。**
>
> **最短读法**：首表 → **§0.3（三项裁决）** → 需要哪章再看 §0.4 的对应小节。
...
| 项 | 值 |
|---|---|
| 正式阶段 | **三个方法章的第一个设计周期全部跑完，Gate 2 实测 0 章过门。** Gate 1 已判不过（D4，09-12 关闭）；C5 低于主锚 1.08 MUC；A4 低于自身消融臂 23.72 causal F1。⇒ 落 `EXPERIMENT_PLAN.md` §5 的 **「≤1 章过」** 分支：**等作者裁决改纲**。 |
| **⚠️ 开工前必读** | 每个新任务先答「科研价值 / 可行性」两问（`CLAUDE.md`「开工自审」节）。不可行**必须点名是数据 / 协议 / 代码 / 算力 / 授权哪一条**，附一手证据，**停下交作者裁决**，不得自行换题绕开。 |
| **活动任务** | ⚪ **无，且队列已空**（2026-09-17 收工）。当周队列五项全部完成：G-5b · E3.1 前置核查 · G-4b · G-11a 核实 · C-3。**执行代理没有可自行推进的任务了。** |
| 🔴 **待作者裁决（三项）** | **① Gate 2 改纲**——0 章过门，三章的负结果现在都有形状（见下一行）。**② Ch6 的上游身份**——E3 契约的 fallback 机制闭合不了，推荐 (乙)。**③ C-3 / G-8 是否执行**——成本已降到 < 1 GPU·h，推荐等 ① 裁完再定。**三项的完整说明见 §0.3**，那是新窗口除本表外唯一必读的一节。 |
...
| **Ch6 是唯一活线** | `EXPERIMENT_PLAN.md` §7.5 明写 Ch6 的构建损失、图依赖正控与不对齐发现**不依赖任何方法章成功**。CSProm-KG 已取得名册第一个 FR-016 **(a)**（WN18RR MRR `0.572682` vs 公布 `0.572660`）。⛔ **但 E3.1 卡住了**，见 §0.3 裁决项 ②。 |
| 论文结构 | 第3章 D4 事实性 · 第4章 A4 关系 · 第5章 C5 身份 · 第6章 E3 图谱构建与下游应用。原 24 条件 factorial / Holm / frozen-vs-finetuned **已撤销，不得恢复**。**章节存废在 Gate 2 判，执行代理不得自行改成两方法章。** |
...
| **gpu-4090** | 🟢 **2026-09-17 起四张卡全空**（实测 24 MiB / 24,564 MiB，0% util，无 compute apps）——此前被他人 vllm 占了一周，现已释放。**但当前没有该占它的任务**：队列已空，Ch6 卡在裁决 ②。 |
...
### 0.3 下一个窗口执行什么：**先拿裁决，没有裁决就没有任务**

**队列已空**（2026-09-17 收工）。当周五项全部完成，结论都已写进 `results/` 并 push：

| 主表 ID | 结论一句话 | commit |
|---|---|---|
| **G-4** | A4.3 四臂收口，`full` causal 7.507508 低于自身消融臂 23.72 ⇒ 第一个设计周期失败 | `ac0da34` |
| **G-5b** | C5 那 1.83 里**误合并占 72.7%**；`full` 的 recall 已超主锚，剩下的 1.084 全在 precision | `df8404b` |
| **G-4b** | A4 的坍塌**无差别**，不是「先丢难例」；损失集中在 PRECONDITION | `e258cc8` |
| **C-3** | LLMERE 根因改写：**11,149 条全部退化重复**，成本 42 GPU·h → **< 1 GPU·h** | `339f974` |
| **E3.1 前置** | 三个方法章的产物与 E3 unit **文档集交集为 0** ⇒ Ch6 卡住，见裁决 ② | `c2f0cbc` |
| **G-11a** | 核实为**早已完成**——三个对手的 (b) 障碍原文一直在 `BASELINE_ROSTER.md` §6.2b | — |

（逐项的执行细节与当时的判断过程见**附录 A**。）

---

#### 🔴 裁决 ① · Gate 2 改纲

**事实**：三个方法章的第一个设计周期都跑完了，**0 章过门**。按 `EXPERIMENT_PLAN.md` §5，
「3 章过」分支在 Gate 1 那天就已失效，实际落点是 **「≤1 章过」⇒ 由作者裁决改纲**
（导师只看结果、不管论文结构，所以「等导师拍板」不是一个能停在那里的状态）。

**现在比上周多了什么**：两章的负结果都有形状了（首表「两章的归因已做完」那一行），
C5 是「方向对、载体弱」，A4 是「机制有害且无差别」，D4 是「主干先输」。**三者性质不同，
摆给导师时不要合并成一句「三个都失败了」**——那会丢掉 C5 唯一的正信号。

**证据页**：`results/PHASE_A.md`（A4）· `results/PHASE_C.md`（C5）· `results/PHASE_D.md`（D4）。

⛔ 执行代理**不得**自行降级论文结构、开新机制家族、启动第二设计周期、调门槛护栏、跑未授权的 seed 17/42。

---

#### 🔴 裁决 ② · Ch6 的上游身份（**这一项不裁，4090 就没有活干**）

**事实（2026-09-17 实测，不是推断）**：E3 evaluation unit 建在 **maven_ere valid 的 437 篇**上，
而 C5/A4 跑在从 train 切出的 **291 篇**、D4 跑在 maven_fact train 的 **2,913 篇**上——
**三者与 unit 的交集都是 0**。`maven_ere valid` 与 `maven_fact valid`（各 710 篇）才覆盖那 437 篇。

⇒ 三份已登记的 `fallback_component_bundle_id` 全是「**另一个 split 上的预测文件**」，
不是能在 valid 上重放的模型。E3 契约 Inputs 假设 fallback 是可直接读的 component bundle，
**两边对 fallback 是什么东西的假设不一致**，缺口在契约层 ⇒ 卡在【数据】+【协议】。

| | 选项 | 代价 | 风险 |
|---|---|---|---|
| **(乙) 推荐** | **沿用历史 `predicted` 图**（v5 判别式抽取器在 valid 上的产物，`.1583` 已发表），表头标明上游身份 | **零**——产物已在，`ch4_sedgpl.pt` 实测 load 后复现 `.1583` 逐位一致 | Ch6 与三个方法章脱钩，但 §7.5 本就说 Ch6 不依赖它们 |
| (甲) | 用三章 checkpoint 在 valid 710 篇上重推理 | GPU 小时级 + 先裁决取哪个臂；D4 是五折 OOF，还要再定口径 | **把 Ch6 绑到三个刚判负的机制上** |
| (丙) | 把 unit 重建到 internal-dev | 违反 E3.0 冻结 | 丢掉与 `.1802/.1583` 并表的能力，不可取 |

**裁完 (乙) 之后 Ch6 就能一路走**：E3.1（CPU，自带 fallback 登记）→ E3.2（CPU，描述性）→
**E3.3（GPU 主表，4090 的活）**。注意 E3.3 要 matched seeds 13/17/42，**多种子仍须逐次授权**。

详见 `results/PHASE_E.md` 的《E3.1 实测阻塞》节。

---

#### 🔴 裁决 ③ · C-3 / G-8 是否执行（LLMERE 重生成）

**事实**：核查改写了根因——不是「95 条格式错」，是 **11,149 条全部退化重复**
（输出中位 1,755 字符 vs 训练 target 98 字符，最短 613，99.65% 超 1,000 字符）。
95 条只是污染落进了第一行，其余 11,054 条污染落在第一个换行之后、被转换器切掉了，
**同一缺陷的两种表现**。根因是生成侧从未继承训练 target 的终止语义。

**成本因此降一个量级**：有用内容（第一行）中位约 10 token，而当前每条生成 450+
⇒ **42 GPU·h → < 1 GPU·h**（线性外推，上卡前先跑 100 条校准）。

**推荐 (甲)：等裁决 ① 之后再定。** 成本已不是障碍，但这个数字有没有去处取决于 Ch4 存废。
方案（①终止条件 ②第一行约束解码 ③全量重生成 ④原始输出保留）已冻结在 `results/PHASE_R1.md` §23，
**通过 ≠ 获准重生成**。
...
| 项 | 值 |
|---|---|
| evaluation unit | **已冻结**（C-10）：`runs/stages/E3/e3-v61-20260913/`，**1,908 实例 / 437 篇 / 候选池 6,892 节点**，`queries.jsonl` `e92629bd…5aecf` |
...
| ⛔ **E3.1 阻塞** | 三个方法章的产物与 unit **文档集交集为 0** ⇒ 见 §0.3 裁决 ② |
