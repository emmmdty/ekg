...
| **活动任务** | 🟢 **G-11a 进行中**：**SimKGC 已收口**（`.6253` / 关重排 `.6373`）；card 0 = CSProm-KG 训到 22/60（约 2.5 h 剩），card 1 = SimKGC 的 description-清空档（约 1 h 剩）。🔴 **收口时读到了一条关于评测单元的事实**：`same_document` 对照 `.8041` / Hit@10 `1.0` ——见 §0.3。 |
...
SimKGC 实测 `.6253`（默认重排）/ `.6373`（关重排）——**两档都低于那条平凡线**。
第三档（description 清空）在 card 1 上训，问的是「去掉文档上下文还剩多少」。
