...
> D4/A4 合并为「负结果与归因」。队首 = 主表 C-11（H1 离线验证，纯 CPU）。**
>
> **最短读法**：首表 → **§0.3（当前队列：C-11）** → 需要哪章再看 §0.4 的对应小节。
...
| 项 | 值 |
|---|---|
| **活动任务** | ⚪ **无在跑的任务，4090 四张卡全空。** 队首 = **C-11（H1 离线验证）**，纯 CPU、不占卡，见 §0.3。 |
...
⇒ **队首 = 主表 C-11（H1 离线验证）**：`full` 的 419 个误合并里 trigger 相似度 ≥0.8 的跨句桶占
**58.0%**（主锚 51.5%），检验误合并是否确实集中在该桶，并算**抑制上界**——按不同阈值拒绝该桶后
`full` 的 MUC precision / recall / F1 各变多少（对照 .756630 / .846422 / 79.90115，主锚 .788430 / .832461 / 80.98472）。
**纯离线、复用 G-5b 产物、不训练不占卡、不动契约。**
若上界连主锚都够不到 ⇒ 抑制侧这条接法本身不够，第二周期要换接法而不是调阈值。

⛔ 第二周期获批的是 **H1 这一条接法**，**不是**新机制家族、不是多种子、不是调门槛护栏。
