...
> 更新于 **2026-09-17（晚，三项裁决已落地 + Ch6 推进一轮）**。本文是新窗口唯一必读入口；
> 读完后再按本文链接打开所需文件，不回溯聊天记录。本文只记录**状态、决策、依赖、下一步**，
> 不复制实验表格——数字只认 [`results/`](results/README.md)。
>
> **现在的状态一句话：三个方法章 0 章过门（不变）；三项裁决已按推荐落地，Ch6 解锁并推完
> E3.1 / E3.2，表 6-2 四行到手。唯一剩下的大件是 G-11a 的四个公开对手行（3–4 周量级）。**
>
...
| **活动任务** | 🟢 **G-11a：SimKGC 全部三档已收口**（`.0563` 无 description ← 主行 / `.6373` 带候选句 / `.6253` 带候选句+默认重排）。card 0 = CSProm-KG 训到 32/60（约 2.7 h 剩），card 1 已空。🔴 **收口时量出一条关于评测单元的事实**：`same_document` 对照 `.8041` / Hit@10 `1.0`，而 SimKGC 的分数 **91% 来自它**——见 §0.3。 |
...
> **工作树状态**：`git status` 应当干净、`HEAD` 等于 `origin/main`（2026-09-17 收工时确认）。
> 最新导师可读周报是 [`reports/2026-09-17_周报.md`](reports/2026-09-17_周报.md)。
> ⚠️ 该周报写于 09-17 **上午**，它第 4 节列的「下周计划」四项**当天下午全部完成或核实为已完成**
> ——下一份周报要从这里接着写，不要照抄那张计划表。
...
### 0.3 下一个窗口执行什么：**G-11a**（表 6-2 只剩四个对手行）

三项裁决 2026-09-17 已按推荐落地（首表第 4 行），队列因此重新有东西。当晚做完的：

| 主表 ID | 结论一句话 | commit |
|---|---|---|
| **裁决 ①②③** | ①改纲推迟到 Ch6 主表之后 · ②Ch6 上游取 (乙) · ③C-3/G-8 维持不执行 | `EXPERIMENT_PLAN.md` 三处 |
| **cgep 修缺陷** | 冻结 unit 有 **68 个 `instance_id` 撞号**（节点下标是每个 ECG 各自的），query edge 改为自己命名自己；重新冻结 `e3-v61-20260917`，逐行零差异 | `ab4ace0` |
| **G-11 / E3.1** | 三层上游接口闭合，`status=closed`；`--relation-dump` 故意无默认值（本地那份是另一份 dump） | `065e46c` |
| **G-11 / E3.2** | 描述性画像：predicted 拓扑边是 gold 的 **3.4 倍**、R2 query F1 **.0795**，而下游 MRR 只掉 **12.2%** ⇒ **R2↔MRR 不对齐** | `6aa73f3` |
| **G-11 / 表 6-2** | 四行到手：random `.0143` / frequency `.0378` / predicted `.1583` / gold `.1802`，Hit@1/3/10/20/50 全列 | `results/PHASE_E.md` |

（逐项数字、口径与 final-valid 披露见 `results/PHASE_E.md` 最后一节。）

---

#### ▶ 队首：**G-11a · 四个公开对手行**（`BASELINE_ROSTER.md` §6.1/§6.2b/§6.2c）

**这是 Ch6 主表唯一还缺的东西，也是本项目当前最大的一件活。2026-09-17 晚已经开工。**

| 已经做完的（可直接接着用） | 在哪 |
|---|---|
| **CGEP → KGC 数据导出** | `scripts/export_cgep_as_kgc.py` → `runs/stages/E3/kgc/CGEP-MAVEN/`（33,017 实体 / 3 关系 / train 46,100 / dev 1,425 / test 1,908 + `test_candidates.txt` 每行 512 个 id）。**断言**：任何一条 query edge 漏进训练图就 fail-fast |
...
🔴 **表 6-2 有一条捷径，读任何文本类对手的行之前先看它**（2026-09-17 实测）：
候选是**全语料**采的而 ECG **从不跨文档** ⇒ 金标后继永远与 anchor 同篇，512 个候选里平均只有
**2.1 个**同篇 ⇒ **只看文档归属就有 MRR `.8041` / Hit@10 `1.0000`**（`same_document` 对照已进
registry 与主表）。**读候选文本的方法吃得到，按 mention token id 打分的（SeDGPL ⇒ 本文两行）吃不到。**
⇒ 跨族比较在这个单元上**不是**在比事件预测；`.1802 / .1583` 不受影响。
**description 消融已收口，把这条量到底了**（判据事先写好）：
SimKGC 带候选句 `.6373` → **清空 description `.0563`，塌 11.3 倍**；
top-1 同篇率 **78.20% → 18.50%**（均匀乱选是 0.41%）⇒ **它那个分数的约 91% 是文档匹配**。
⇒ **表 6-2 的 SimKGC 主行取「无 description」档**（与 SeDGPL 族同轴：两边都看不到文档身份），
带 description 那档并列入表但标成「吃捷径档」，和 `.8041` 挨着放。

**⇒ 在那条诚实的轴上：本文构建图 `.1583` 是 SimKGC `.0563` 的 2.8 倍**，
高于 frequency `.0378` 与 random `.0143`。⚠️ 单 seed，且 SimKGC 的 batch 被显存压到 256
⇒ **那一行仍是它的下界**，写结论必须带这两条限制。
...
CSProm-KG 按**实体嵌入表**打分 ⇒ 被系统性推离正确答案；SimKGC 是**文本双编码器** ⇒ 几乎不受影响
（**它可能才是更适配 CGEP 的那个 KGC 对手**，尽管已发表数字最低）。
**数字照报、解释照写，不得换映射凑分。**

**先记住这条结构性事实**（§6.2a 已核实，不要重新调研）：**四个对手没有一个实现 CGEP**。
它们实现的是各自原任务（MCPredictor/BART contrastive = MCNC 五选一；CSProm-KG/SimKGC = 知识图谱
补全），SeDGPL 作者做的 CGEP 适配**从未发布**。⇒ 表 6-2 那四行在我们的重建协议上**注定是
(b) 透明适配**，适配代码要我们自己写，且**必须逐条列出与原设置的差异**。
(a) 只能在各自的原基准上取得，用来证明我们把方法跑对了——CSProm-KG 已经取得
（WN18RR MRR `0.572682` vs 公布 `0.572660`），那是唯一一个。

**开工时先答两问**（`CLAUDE.md` 开工自审），并且**逐个对手分别答**——四个的可行性不一样：

| 对手 | 已知障碍（一手，§6.2b） | 先判什么 |
|---|---|---|
| **CSProm-KG** | 无阻断，(a) 已取得，仓库在 `gpu-5090:/mnt/aidata/tongjiakai/baselines/CSProm-KG`，独立 venv | **队首候选**：只差「把 1,908 个 CGEP query 变成它吃得下的 KGC 形式」这一层适配 |
| **SimKGC** | 原配置 `--batch-size 1024` 需 4×32 GB；减 batch 会改 in-batch 负样本数，按定义即非其发表设置 | 适配后**只能报 (b) 并写明 batch 差异**；别为了凑 (a) 去改卡 |
| **BART contrastive** | `torch==1.7.1` + apex 版本墙；另需两阶段预训练 | 先判版本墙在 cu128 栈上能不能绕（**这是【代码】可行性问题，不是进度问题**） |
| **MCPredictor** | 需 LDC2011T07 Gigaword 许可 + python2.7 预处理链 | 许可拿不到就是**【数据】不可行**，点名 + 落 (b) + 停下，不要自己找替代语料 |

**排期基准率**：§3.1 给的是 **3–4 周**（G-11a 原估）。**不要按「计算耗时」估**——按 git 里同类事件
的真实起止天数估（`ekg-estimate-from-base-rates`）。

**硬前置（别踩）**：
1. 所有对手**必须跑在冻结 unit `runs/stages/E3/e3-v61-20260917/`（`f75e7e87…`）上**，
   与 `.1802/.1583/.0378/.0143` 并表。用旧的 `e3-v61-20260913` 就白跑；
2. **外部仓库、预训练权重、公开 checkpoint 一律本地下载后 `scp`**（§0.6 的域名表），
   服务器上不要试 `git clone` 或 `from_pretrained`；带宽约 0.4 MB/s，排期先把搬运算进去；
3. 每个适配都要记**透明补丁 + 前后 hash**（FR-016 (b) 的要求），写进 `results/PHASE_E.md`；
4. ⛔ 服务器上不跑 `uv run` / `uv sync`；4090 用 `.venv/bin/python`。

#### 顺位第二（可并行，CPU 或小 GPU）

- **E3.5 叙事闭环**：三条结论已经全部有数字（相对对手的位置要等 G-11a；构建代价 −.0218 = 整图
  价值 22.0%；不对齐现在有**两对**——causal_scc↔R1 的 ρ=−0.064，和新测的 **R2↔MRR**）。
  可以先把前两条以外的部分写进结果页；
- **E3.4 的受控扰动曲线**已完成（表 6-4），**不重跑**。

#### ⛔ 一律不做（不变）

D4 第二个周期、调 threshold、扫 epoch、换 split、加大 backbone、用 seed 17/42 去捞 D4、
把探测数字写进主表或与 33.17 / 32.10 相减、为了让 C5.3 跑通去放宽那条 fail-fast、
自行降级论文结构、开新机制家族、启动任何章的第二设计周期。

### 0.4 三章终态（数字唯一权威在 `results/`，这里只给结论与去向）

三章的第一个设计周期**全部结束**，没有一个在跑。**三者的失败性质不同，不要合并成一句话。**
...
#### 0.4c Ch6（E3 / G-11a）：**唯一活线**，但 E3.1 卡在契约缺口

| 项 | 值 |
|---|---|
| evaluation unit | **已冻结**（C-10，09-17 重新冻结）：`runs/stages/E3/e3-v61-20260917/`，**1,908 实例 / 437 篇有实例（候选池覆盖 607 篇）/ 6,892 候选节点**，`queries.jsonl` `f75e7e87…a11e6` |
| CSProm-KG | ✅ **FR-016 (a) 已取得**（09-16）：WN18RR MRR **0.572682** vs 公布 0.572660，H@1/3/10 四项全在**事前登记**的容差内。仓库在 ekg 之外（`gpu-5090:/mnt/aidata/tongjiakai/baselines/CSProm-KG`），独立 venv，五处透明补丁与前后 hash 见 `results/PHASE_E.md` |
| 其余三个对手 | **维持 (b) 并已写明障碍**（`BASELINE_ROSTER.md` §6.2b），不再投入复现 |
| ✅ **E3.1 已闭合** | 裁决 ② 取 (乙)：`predicted` 上游 = **v5 判别式抽取器在 valid 上的产物**，不是 C5/A4/D4（那三章与 unit 文档集交集实测为 0）。三层接口 `status=closed`，`upstream_registry.json` `f2685e10…6e9e` |
| unit 版本 | ⚠️ **已重新冻结为 `runs/stages/E3/e3-v61-20260917/`**（`f75e7e87…`）——旧的 `e3-v61-20260913` 有 68 个 `instance_id` 撞号。逐行零差异、`candidate_id_digest` 不变，所以与 `.1802/.1583` 并表的能力没丢。**新对手一律跑在新 unit 上** |
| 表 6-2 进度 | 4/8 行到手（random / frequency / predicted / gold，Hit@k 全列）。缺的是四个公开对手行 = **G-11a** |
...
- **最新有记录的三件套：639 passed / 29 skipped、ruff 0、`ekg-smoke` OK**（2026-09-16，
  对应 C5 推理侧修复 `a1e1509`，记在 `results/PHASE_C.md`；前值 635/29 对应 `5cfbeff`）；
- ⚠️ **一处可追溯缺口**：`a1e1509` 之后还有两个代码提交——`a65f456`（`prepare_c5_argument_uncertainty_preflight.py`）
  与 `791a03a`（`prepare_a4_pair_evidence_preflight.py`），各改一个 preflight 脚本，
  **没有单独的三件套记录**。两者都已被各自重建的 preflight 哈希钉住并实跑通过（C5 `7a56e451…b6b0`、
  A4 `11e4343a…490c`），但**下次开工时顺手补跑一次三件套**，把这个缺口补上；
- **2026-09-17 的全部提交（`ac0da34` 起共 10 个）只动 `docs/`，没有一行代码改动**；
...
⚠️ **2026-09-17 实测：那条「E3 等三类上游 handoff」的箭头接不上。** 三个方法章的产物与 E3
evaluation unit **文档集交集为 0**（unit 在 maven_ere valid 的 437 篇，C5/A4 在 train 切出的 291 篇，
D4 在 maven_fact train 的 2,913 篇），三份已登记的 fallback 全是「另一个 split 上的预测文件」。
⇒ **依赖图上这条边在契约层是断的**，怎么接由作者裁决（§0.3 裁决 ②），推荐 (乙) 沿用历史 predicted 图。
...
| **gpu-4090** | 有空即可自用（本地三件套全绿后无需逐次点头）；**不挤占他人正在跑的卡**。🟢 **2026-09-17 起四卡全空**（实测 24 MiB / 24,564 MiB，0% util），此前被他人 vllm 占了一周。~~A4.3 必须在这台机上~~ **已被作者 2026-09-16 裁定推翻**（「4090 不可行就使用 5090」），A4.3 已在 5090 跑完；跨机比较的代价以限制说明形式随结果报，见 §0.4a。 |
