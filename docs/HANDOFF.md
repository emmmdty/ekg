...
> 错误归因与可行性诊断都已做完（§25.21–§25.23）。**卡 0/1/2 已释放，无在跑任务。**
...
| 🔴 **队首（要裁决）** | **D4 第二个实质不同的设计周期投不投、投哪条。** 两条路的可行性已先于裁决摆清（§25.22）：**(甲) 抬边精确率 ⇒ 走不通**（精确率 `.50` 封顶，那时触及面只剩 3.2%，要换更强的关系抽取器＝跨章依赖）；**(乙) 机制只在结构足够时介入 ⇒ 本章内可行，但线索是事后分层，必须回 R1 重新预注册**。⛔ 不得换名、扫参、加大 backbone、启动未授权多种子。 |
