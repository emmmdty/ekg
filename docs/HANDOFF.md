...
| **活动任务** | ⚪ **无。** 三条方法线的第一个周期全部收口；**09-17 下午做完四项离线工作**——**G-5b C5 差距归因**（`df8404b`）· **E3.1 前置核查与实测阻塞**（`c2f0cbc`）· **G-4b A4 误差分析** · **G-11a 核实为已完成**。**09-17 收工时队列已清空**——G-5b · E3.1 核查 · G-4b · G-11a 核实 · C-3 全部完成。**剩下的全是待作者裁决项（见下一行），执行代理没有可自行推进的任务。****没有作者裁决之前不要开新任务**——不开第二设计周期、不换机制家族、不跑 seed 17/42。 |
...
| **待作者裁决** | 🔴 **两项。第二项 2026-09-17 新增：Ch6 的上游身份**——实测三个方法章的产物与 E3 evaluation unit **文档集交集为 0**（unit 在 valid 437 篇，C5/A4 在 train 切出的 291 篇，D4 在 maven_fact train 2,913 篇），三份已登记的 fallback 全是「另一个 split 上的预测文件」，搬不过去 ⇒ **E3 契约的 fallback 机制闭合不了**，卡在【数据】+【协议】。**推荐 (乙)：沿用历史 `predicted` 图**（Phase A 抽取器在 valid 上的产物，零成本、不依赖任何失败机制，表头标明上游身份）；(甲) 用三章 checkpoint 重推理会把 Ch6 绑到三个没过门的机制上。三个替代与实测表见 [`results/PHASE_E.md`](results/PHASE_E.md)。**第一项：Gate 2 的改纲决定**（`EXPERIMENT_PLAN.md` §5「≤1 章过」分支）。两页结果已摆好：`results/PHASE_A.md`（A4）· `results/PHASE_C.md`（C5）· `results/PHASE_D.md`（D4）。**改纲只关乎章节形态，不阻塞第 6 章复现、两章的离线归因与文档审查**（§0.3 队列 1–4 项照跑）。 |
