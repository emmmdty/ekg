...
| **活动任务** | **E8：LLMERE-causal 透明适配 baseline**（`wip`；SFT adapter 已保留，完整 metrics extras 已核验；PID `1819697` 在 GPU0 对 11,149 条 internal-dev 样本生成，尚无生成文件、评分或指标）与 **E11：D4 typed-cue 准备**（`wip`；仅 D4.0–D4.2，先本地实现/预检/烟测；未经其全部门与重新核卡不启动 pilot）。A4/D4 已由 E6/T024 冻结；C5 因 E10 判定 ACCI 不可运行，仍需作者另议第二方法族。**执行顺序认表里的行序，不认编号大小** |
