...
| **活动任务** | ⚪ **无在跑的任务，四张卡全空。G-11a 第一轮收口完毕**：表 6-2 的**三个公开对手全部到齐**（SeDGPL `.1802` / SimKGC `.0563` / CSProm-KG `.0028`），加两个平凡对照与两行协议证据。**下一件事见 §0.3。** |
...
### 0.3 下一个窗口执行什么：**E3.5 收尾，然后裁决 ① 该落地了**

**G-11a 第一轮 2026-09-18 凌晨收口。表 6-2 成形，三个公开对手到齐。**

| 方法 | MRR | Hit@1 | Hit@10 | Hit@50 |
|---|---:|---:|---:|---:|
| CSProm-KG[Chen+ 2023]（(b)，⚠️ **结构性不兼容**） | .0028 | .0000 | .0000 | .0000 |
| random | .0143 | .0021 | .0241 | .0891 |
| frequency | .0378 | .0267 | .0372 | .1509 |
| SimKGC[Wang+ 2022]（(b)，batch 256，无 description） | .0563 | .0183 | .1279 | .3375 |
| **本文构建图（predicted 上游 = v5 判别式抽取器）** | **.1583** | .1038 | .2563 | .5126 |
| **SeDGPL**[Zhan+ 2024]（(b)，本项目自跑）＝ gold 上界 | **.1802** | .1143 | .3124 | .5823 |
| —— 下面两行测的**不是**事件预测，只作协议证据 —— | | | | |
| SimKGC（**+ 候选句** ⇒ 吃捷径档） | .6373 | .5079 | .8585 | .9308 |
| `same_document`（只看文档归属） | .8041 | .6567 | **1.0000** | 1.0000 |

🔴 **读这张表必须带的两条**（都已量出机制，不是推断，见 `results/PHASE_E.md`）：

1. **`same_document` = .8041 / Hit@10 1.000**——候选全语料采、ECG 不跨文档 ⇒ 金标永远与 anchor
   同篇而 512 个候选里平均只有 2.1 个同篇。**读候选文本的方法吃得到，按 mention token id
   打分的（SeDGPL ⇒ 本文两行）吃不到。** SimKGC 带候选句 `.6373` → 清空 description `.0563`
   （塌 11.3 倍），top-1 同篇率 78.20% → 18.50%（均匀乱选 0.41%）⇒ **它 91% 的分是文档匹配**；
2. **CSProm-KG 朝相反方向失败**：按实体嵌入表打分，而**金标在训练图里 0/1,908 有边**
   （CGEP 的 query edge 规则要求尾节点 outdeg 0 / indeg 1）。有训练边的候选平均排名 **195.0**、
   没有的 **399.7**（乱排 255.5）⇒ Hit@50 恒 0。**内部对照**：同 checkpoint 在 dev 上
   `val_mrr=0.2907`（还是在全部 33,017 实体里排）⇒ 模型没坏。**也不是训练不足**——
   未见实体只收负梯度，多训只会拉大差距。

⇒ **论文里不得写「我们超过了 CSProm-KG / SimKGC」**。能写的是：本文构建图在这批 query 上
高于两个公开 KGC 对手与两个平凡对照，**而两个对手的落后各有已量出的结构性解释**。
**限制**：单 seed；SimKGC batch 被显存压到 256；CSProm-KG epoch 取 60。

---

#### ▶ 队首 1：**E3.5 叙事闭环**（CPU，无授权项）

三条结论现在全部有数字了，可以直接写 `results/PHASE_E.md` 的收尾节：
