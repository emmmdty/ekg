...
> 更新于 **2026-09-16**。本文是新窗口唯一必读入口；读完后再按本文链接打开所需文件，不回溯聊天记录。
...
**A4.3 必须在 4090 跑**：它的判定线含 A3 fallback **32.10**，那是在 4090 pin（`71be7419…`）上训的；
G-13 实测同一臂在 5090 backbone 上是 **31.13** ⇒ 换机就引入混淆。**C5 能搬而 A4 不能，别搞反。**
